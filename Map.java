...
public class Map implements ActionListener{
    ...
    /** If (x, y) is in the grid and empty, put a new piece of type type in location (x, y); and,
     if the new piece is J*Man, store it in field jMan.
     Precondition: type is one of the piece constants in class Piece.*/
            ...
            for ( int i= 0; i < m.width; i= i+1 ){
                for ( int j= 0; j < m.height; j= j+1 ){
                    ...
                    if ( m.grid[i][j] != null && m.grid[i][j].getType() == Piece.BLOCK ){
                        ...
                        g.setColor( m.grid[i][j].getColor() );
                        g.fillPolygon( new int[]{h+1, h1-1, h+m.tileWidth/2}, new int[]{v1-2, v1-2, v}, 3 );
                    }
                }
            }//end of for loops
        }
